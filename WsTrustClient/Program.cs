...
namespace WsTrustClient
{
    class Program
    {
        static string _stsAddress = "https://127.0.0.1:5443/wsTrust13/transportIssueSaml20";
        ...
        static void Main(string[] args)
        {
            ...
            stsBinding.Security.Message.ClientCredentialType = MessageCredentialType.IssuedToken;
            BindingUtilities.SetMaxTimeout(stsBinding);

            SecurityToken token = null;
            var trustChannelFactory = new WSTrustChannelFactory(stsBinding, new EndpointAddress(_stsAddress));
            ...
            try
            {
                ...
                {
                    AppliesTo = new EndpointReference(_serviceAddress),
                    TokenType = TokenTypes.Saml20,
                    KeyType = WSTrust13Constants.KeyTypes.Symmetric
                };

                var trustChannel = trustChannelFactory.CreateChannelWithIssuedToken(genericXmlToken);
                token = trustChannel.Issue(rst, out RequestSecurityTokenResponse response);
            }
            catch (Exception e)
            {
